    ...
    if debug:
        ...
        if args.exp_mode in ['CTR', 'SGL']:
            ...
            if args.active_mode == 'sprtrelu':
                ...
                # [新增] 兜底逻辑，处理 softmax 或其他模式
                file_name = f"{args.num_clients}p_{args.divide}_in{args.lag}_out{args.horizon}_{args.active_mode}_{args.local_epochs}locals_{'fedavg' if args.fedavg else 'nofedavg'}.log"
        else:
            # 防止未知的 exp_mode 导致 crash
            file_name = "unknown_experiment.log"
...
if __name__ == '__main__':
    ...
    args.lag = 12
    args.horizon = 12
    args.active_mode = 'softmax' # 测试默认情况
    args.local_epochs = 2
    args.fedavg = True
    
    logger = get_logger(args, './', debug=False)
